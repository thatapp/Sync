...
const { process } = require('../../lib/actions/updateSingleDocument');
...
describe('"Update single Document" action test', () => {
  describe('succeed', () => {
    let execRequest;
    beforeEach(() => {
    ...
    it('process', async () => {
      ...
      expect(execRequest.getCall(0).args).to.be.deep.eq(['updateOne', { filter: body.condition, update: { $set: body.save_data }, upsert: true }, { body }]);
    });
  });
  describe('fail', () => {
    beforeEach(() => {
