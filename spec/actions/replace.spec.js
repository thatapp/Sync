...
const { process } = require('../../lib/actions/replace');
...
describe('"Replace a Document" action test', () => {
  describe('succeed', () => {
    let execRequest;
    beforeEach(() => {
    ...
    it('process', async () => {
      ...
      expect(execRequest.getCall(0).args).to.be.deep.eq(['replaceOne', { filter: body.condition, replacement: { $set: body.save_data }, upsert: true }, { body }]);
    });
  });
  describe('fail', () => {
    beforeEach(() => {
