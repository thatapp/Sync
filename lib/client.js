...
class Client {
  constructor(context, cfg) {
    ...
    this.axiosClient = axios.create({
      baseURL: `https://data.mongodb-api.com/app/${appId}/endpoint/data/${apiVersion}/action`, //Old Url
      // baseURL: `https://eastus2.azure.data.mongodb-api.com/app/${appId}/endpoint/data/${apiVersion}/action`, //New Url
      headers: {
        "api-key": apiKey,
  ...
  async apiRequest(action, additionalData, msg) {
    ...
    try {
      ...
      console.log(err);
      this.logger.error(
        `Got error "${err.response.statusText}", status "${
          err.response.status
